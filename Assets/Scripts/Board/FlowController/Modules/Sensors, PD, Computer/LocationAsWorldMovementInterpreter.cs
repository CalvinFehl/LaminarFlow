...
public class LocationAsWorldMovementInterpreter : MonoBehaviour, IHandleRigidbodyData
{
    ...
    private void FixedUpdate()
    {
        if (autoUpdate)
        {
            ReceiveLocationTracking(transform.position, Time.fixedDeltaTime);
        }
    }
    ...
    public void ReceiveLocationTracking(Vector3 currentLocation, float deltaTime)
    {
        ...
        Vector3[] values = LocationAsWorldMovementAccumulationSort(deltaLocation, deltaTime, lastDeltaLocation, locationAsWorldMovementEventData.AccumulatedLocation, locationAsWorldMovementEventData.AccumulatedLocationTime);

        locationAsWorldMovementEventData.DeltaLocation = deltaLocation;
        locationAsWorldMovementEventData.LocationAcceleration = deltaLocationAcceleration / deltaTime;
